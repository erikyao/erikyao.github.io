...
[const-function]: /assets/posts/2015-03-29-cpp-const-object--const-member--const-member-function--mutable/const-function.jpg
...
When you create an ordinary (non-`static`) `const` member inside a class, you CANNOT give it an initial value. This initialization must occur in the constructor (constructor 意味着 runtime), but in a special place in the constructor called **constructor initializer list**.
...
上一节说道：A `const` member function CANNOT modify any non-`static` data members，那对一般的 non-`static` + non-`const` 的 vanilla member 而言，可以开一个后门使其被 `const` member function 修改，这个后门就是 `mutable`.

其实还有一个方法可以使 vailla member 被 `const` member function 修改：因为 `const` member function 内的 `this` 指针实际是一个 `const` pointer，我们可以用 `const_cast` 把 `this` 去 `const`，然后直接访问 vanilla member 并修改。写出来就是 `(const_cast<T*>(this))->i = 47;`。这个招式实在是太暴力了，为我武林正派人士所不齿。
{: .notice--info}
...
* `static` member 可以修改（反正不影响 object 状态）
	* `const static` member 因为是 const 所以无法修改

...
- Unix: COFF (Common File Format) 格式
...
- executable 被装载 (loaded) 后，`.code` 和 `.data` 可以被映射到不同的 virtual memory segment，`.code` 可以设置为 read-only，防止被修改
- CPU 的 cache 一般都设计为 instruction cache 和 data cache 两部分，sectioning 有利于提高 CPU cache 命中
- 可以实现一个 executable 的多个副本共享指令，节省内存 
...
- **Calling Convention**, which controls how functions' arguments are passed and return values retrieved. E.g.
...
- System Call 的具体实施方案。E.g. 
    - 选择 direct system calls 还是 procedure calls to system call stubs
    - 指定 system call numbers
...
一个 SLL 可以简单地看做一组 object files 的集合。

比如 `glibc`，它是由 C 语言开发的，编译后得到 `printf.o`、`scanf.o`、`fread.o`、`fwrite.o`、`data.o`、`time.o`、`malloc.o` 等 object files 供开发者使用。为了方便管理，通常用 `ar` 压缩程序将这些 object files 打包成 `libc.a` 这个 SLL 文件。

经常会有这样的情况：一个 object file 里只包含一个函数。这么做是为了减少 `#include` 导入多余的内容造成空间的浪费。

### 4.6 Linking 过程控制

OS Kernel、BIOS (Basic Input Output System)，或者一些在没有 OS 的情况下运行的程序 (比如 Boot Loader、PQMagic)，或者嵌入式的程序会有一些特殊的 linking 要求，比如要指定输出文件的 section 的地址、section 名称、section 存放顺序等。
...
- 这明显是一个没搞懂数学而误用的词
- PE 文件在 being loaded 时直接映射到 process 的虚拟空间中运行，按照 executable $\rightarrow$ process 的方向，和 image 的定义 ($f(x) = y$ is the image of argument $x$ under $f$):
    - PE is the preimage
    - Process is (or contains) the image (of the PE file)
- 但 PE executable 在 Window 语境下也叫做 image file (映象文件)，我就很无语

Linux 还有 ["process image"](https://tldp.org/LDP/LG/issue23/flower/psimage.html) 这个概念，它本质是 process 的结构的描述，你可以理解成 process 的 memory layout，它基本等同于 executable 的 layout。这个叫法是符合数学意义的，但它本身似乎没啥必要 (同样的 layout，只不过一个在 file 里，一个在 memory 里)

# Part 3. 装载与动态链接 (Loading & Dynamic Linking)

## 6. Executable Loading / Process

executable 只有被 load 到 memory 后才能被 CPU 执行。早期的 loading 十分简陋，基本过程就是把 executable 从 disk 读取到 memory 中的某个位置。随着 MMU 的诞生和 multi-process、虚拟内存的出现，loading 变得复杂起来。

### 6.1 Process 的虚拟地址空间 (VAS, Virtual Address Space)

Executable vs Process：

- executable 是一个静态的文件
- process 是 executable 运行的一个动态过程
- 若把 executable 比作菜谱，process 就是做菜的过程。把一道菜同时炒两份就是 multi-process。

process 有自己独立的 VAS。32-bit 系统的 VAS 有 4GB。
...
Problem: 如果 executable 需要的内存大于物理内存？

1. 方法一：加内存
2. 方法二：executable 运行时是有局部性原理 (Principle of Locality) 的，所以可以将最常用的部分放内存，不太常用的部分放磁盘。是所谓 _**动态装载**_ (dynamic loading)。
...
- 首先把 executable 所有的内存划段 (做一个 layout) 
...
- 当 load 新 page 进来时，需要替换掉旧的 page
    - 此时可以有多种算法来决定替换哪个 page，比如：FIFO

### 6.4 Process 的 VAS 的分布

Section / Segment / VMA (Virtual Memory Area) 的区别与联系：

- section:
    - object file 中用 section directive 来标记 sections，比如 `.text`, `.data`, `.bss`, `.rodata`。这些 directive 本身就是 object file 的一部分
    - executable 中，section 的物理结构和 object file 中的一致，同时有 section 的描述信息 (比如 names, sizes, and offsets) 构成 **section header table**，也是 executable 文件的一部分
- segment:
    - 由 linker 产生，只存在于 executable 中
    - executable 中的 section 的物理结构并没有发生变化，只是多了一个 segment 的描述信息，构成 **program header table**，成为 executable 文件的一部分
...
- VMA:
    - 本质是 VAS 中的一个 memory segment 
        - Linux 中叫做 VMA，Windows 中叫做 Virtual Section，概念上是一样的
    - 在 loading 时，executable 中的 segment 会 load 到独立的 VMA 中
        - 这样做的一个好处是减少 page 的浪费

stack 和 heap 是两个特殊的 VMA，它们并没有映射到文件中的某个 segment，所以属于 Anonymous VMA。

一个 process 基本有一下几种 VMA：
...
#### 6.4.5 Stack 初始化
 ...
 process 刚开始启动时，需要知道一些信息比如环境变量 (`PATH=/usr/bin` 这类的) 和命令行参数 (`argv`)。很常见的一个做法是 OS 在 process 启动之前将这些信息保存到 process 的 stack 中。 process 启动之后，这些信息会被传给 `main()`

### 6.5 Linux Kernel 装载 ELF 过程简介

当 bash 下输入命令执行某个 ELF 时：

- 首先 bash 会先 `fork()` 出一个新 process 
- 新 process 调用 `execve()` 执行 ELF，开始装载
    - `execve()` 在 kernel 中的入口是 `sys_execve()`，位于 `kernel/Process.c`
...
- 更新起来很麻烦
    - 比如 `a.o` 依赖 `lib_1.o`、`lib_2.o`……`lib_n.o`，你更新其中一个 lib，整个 `a.o` 就需要重新 link
...
- 如果此时再 load `b.o`，我们可以先检查内存，发现已经有一个 `lib.so`，就不用再 load `lib.so`，直接 link
    - 实际上 `lib.so` 的 DATA segment 还是会有两份，这样两个 process  `a.o` 和 `b.o` 对 `lib.so` 全局变量的修改就可以被隔离开
        - 多 process 共享全局变量也是可以做到的，用到的技术叫 "共享数据段"
    - 同一 process 的两个线程访问是同一个 DATA segment
...
- 加快 executable 启动速度，特别是对依赖大量函数引用的 executable 
...
## 9. Windows 下的 Dynamic Linking

略

# Part 4. Lib / Runtime Lib (运行库)
...
### 10.1 Executable 的内存布局

32-bit 系统有 4GB 内存空间，除掉 kernel 占用的部分 (Kernel Space)，剩下的都是 User Space。

一个应用 executable 的内存空间有以下默认的区域：

- stack：用于维护函数调用的 context，离开了 stack 函数调用就没法实现。
    - stack 通常在 executable 所占内存的最高位地址分配
- heap：供 executable 动态分配的内存区域，比如 `malloc()` 或者 `new`
    - 通常位于 stack 的下方 (低地址方向)，一般比 stack 大很多
- executable 映象
- 保留区：受到保护而禁止访问的内存区域
- dynamic libraries 映射区：用于装载 DLL

### 10.2 Stack 与 Function Calling Convention
...
- 策略一：交给 kernel 去做。每次申请或者释放，都去 invoke system call 
    - 缺点：system call 的开销是很大的，频繁 invoke system call 会影响性能
- 策略二：runtime lib 代管。runtime lib 向 OS “批发” 一块空间 (i.e. heap)，然后向应用程序 “零售”。如果全部用完了再向 OS “进货” (invoke system call ，得到一个新的 heap)。
...
Windows 下，当我们用 `__declspec(thread)` 申明一个 TLS 变量时，编译器会把这个变量放到 PE 文件的 `.tls` section。当系统启动一个新 thread 时，它会从 process 的 heap 中申请一块空间，然后把 `.tls` section 的内容复制到这块空间中。
...
## 12. 系统调用 (System Call) 与 API

### 12.1 System Call 介绍

#### 12.1.2 Linux System Calls

x86 下，system call 由 `0x80` 中断发起，`EAX` 寄存器表示 system call 的接口号，比如：

- `EAX = 1` 表示 “退出 process ”，在 `/usr/include/unistd.h` 中定义为 `exit()` 函数，这个函数会调用一个 kernel function `sys_exit()`；
- `EAX = 2` 表示 “创建 process ”，对应 `fork()` 和 `sys_fork()`；大抵就是这么一个规律。

runtime lib 的部分工作就是 invoke 了 system call，并在 invoke 前后帮你做了很多后勤工作，这也算是一种 “屏蔽底层技术细节”

- 比如 `read()` system call 读取的就是文件的原始数据，而 runtime lib  `glibc` 的 `fread()` 就提供了 buffer、按行读取这些便利

而且 runtime lib 有 “标准库” 保障，算是 “跨平台” 的 “屏蔽底层技术细节”
...
System call 都运行在 kernel mode，应用程序一般运行在 user mode。应用程序 (通过运 runtime lib) 调用 system call 时，OS 会通过中断从 user mode 切到 kernel mode

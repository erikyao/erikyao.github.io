...
An estimator $\hat \Theta_m$ is said to be (weakly) consistent if it converges to $\theta^\ast$ in probability, i.e.:
...
$$
\underset{m \to \infty}{\lim} \mathbb{P}(\lbrace \lvert \hat \Theta_m - \theta^\ast \rvert > \epsilon \rbrace) = 0
$$

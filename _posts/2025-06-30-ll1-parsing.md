...
- if $b \in \Phi[Y_1]$, then $Y_1 \overset{*}{\Rightarrow} b \dots$, and then $X \overset{*}{\Rightarrow} b \dots Y_2 \dots Y_k$. Therefore $b \in \Phi[X]$, and consequently $\Phi[Y_1] \subseteq \Phi[X]$
- if $Y_1$ is nullable, then $X \overset{*}{\Rightarrow} Y_2 \dots Y_k$. Logic falls back to above.
...
To construct PPT, we need $\Phi[\alpha]$ for each RHS $\alpha$ appearing in a production. This algorithm is similar to [Section 3.1.2](#312-pilot-algorithm):

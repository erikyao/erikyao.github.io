...
In this post we use the following abbrevation, symbols, and operators:
...
- $\Phi$ is a map of $\alpha \mapsto \operatorname{FIRST}(\alpha)$ where $\alpha$ could be:
    ...
    - a RHS sequence as in any production $(X \to \alpha) \in P$
...
The verification can also be done from PPT.

# 3. How to compute PPT

In 2 out of the 3 cases we listed above, we need to compute PPT. It needs do some groundwork,  specifically, the $\operatorname{FIRST}$ and $\operatorname{FOLLOW}$ sets for each non-terminal $X \in V$ in a gammar $G=(V, \Sigma, S, P)$.
...
Some textbooks define **nullable** property as:
...
### 3.1.2 Algorithm: computing $\operatorname{FIRST}$ set for a single symbol, when other $\operatorname{FIRST}$ sets are known

We can use the following algorithm to compute $\operatorname{FIRST}$ sets:

$$
\begin{align}
&\textbf{procedure } \mathrm{compute\_first\_set\_for\_symbol}(\forall X \in V \cup \Sigma, \Phi) \text{: } \nonumber \qquad \newline  
...
& \qquad\qquad\qquad \text{fetch } \Phi[Y_i] \text{ from } \Phi \qquad \newline 
...
- $\Phi$ is a map of known $\operatorname{FIRST}$ sets, except $\Phi(X)$ we are going to compute
...
Here we want to compute all $\operatorname{FIRST}$ sets at the grammar level with a single function. 
...
PPT is a table whose row-index is $V$, column-index is $\Sigma$, and each cell is a production $\in P$.

$$
\begin{align}

&\textbf{global } \Phi = \mathrm{compute\_first\_sets\_for\_V}() \qquad \newline
& \qquad \quad \; \Phi \uplus \big \lbrace \mathrm{compute\_first\_set\_for\_RHS}(\alpha) \mid (X \to \alpha) \in P \big \rbrace \qquad \newline
...
1. All the three algorithms on $\operatorname{FIRST}$ sets are way simpler
    - See [Section 3.1.2](#312-algorithm-computing--set-for-a-single-symbol-when-other--sets-are-known) as an example
2. No need to compute $\operatorname{FOLLOW}$ sets at all!
3. Line (1)(2)(3) in above algorithm can be removed

Howeve as we mentioned in [Section 3.1.2](#312-algorithm-computing--set-for-a-single-symbol-when-other--sets-are-known), in real world, [$\varepsilon$-production are not always elimimated](/compiler/2025/06/24/appetizers-before-parsing-serving-order).
...
Other top-down parser implementation techniques can be found in [](/compiler/2025/06/26/top-down-parsers-recursive-descent-predictive-and-more)